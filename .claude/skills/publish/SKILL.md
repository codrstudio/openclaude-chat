...
3. `.github/workflows/publish-tgz.yml` — workflow que gera e sobe o `.tgz` tarball como asset na GitHub Release.
...
- Antes do passo 3 (confirmacao), **mostrar** ao humano o nivel inferido + os commits que justificam a decisao. O humano pode corrigir respondendo `major`/`minor`/`major` em vez de `yes`.
...
- A tag push ativa **todos** os workflows (`publish-npm.yml`, `publish-github.yml` e `publish-tgz.yml`)
- Monitorar as tres runs em paralelo:
  ...
  - `gh run watch` na run do `publish-tgz.yml` (tarball -> release asset)
...
- **Release tarball**: `gh release view v<version> --json assets` — confirma que o `.tgz` existe como asset
